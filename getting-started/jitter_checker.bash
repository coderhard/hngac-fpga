#/usr/bin/bash
# Purpose: Create a jitter capture tool.
# Why: In robotics, "Average" is vanity; "Max" is sanity. A robot that works "on average" but freezes for 
#      50ms once every minute will crash.
#
#      To get these metrics (p95 and Max) into your paper, we have to deal with the 
#      Observer Effect: the stopwatch (std::chrono) takes ~20-50ns to click, almost as long as our auth check (38ns).
#
# This updated version, ngac_jitter.cpp, accepts the "measurement overhead" in exchange for capturing the variance.

# 1. Create the Jitter Test Code
# This version pre-allocates an array to store every single run's time (obviously a memory hit), then sorts them at 
# the end to find the percentiles.

#Command to create the file:
cat << 'EOF' > ~/projects/robo-ngac-hypergraph-experiment/benchmarks/ngac_jitter.cpp
#include <bitset>
#include <chrono>
#include <iostream>
#include <vector>
