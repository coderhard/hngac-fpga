...
In software, those same dimensions make every policy rule more expensive, the fifth by 19\%. The two added dimensions cost 1{,}336 LUTs and no BRAM or DSP. With 5D H-NGAC, robotics and functional-safety engineers can add a security dimension to the control path without spending a clock cycle.
\end{abstract}
...
\textbf{Software policy engines and models.} The NGAC standard and the Policy Machine define the model we compile, but report no per-decision latency \cite{incits565, ferraiolo2011policymachine, hu2014sp800162}. Ferraiolo et al.\ establish NGAC's decision and review advantages over XACML \cite{ferraiolo2016xacml}, and every NGAC implementation to date is software, including NIST's fine-grain enforcement over database queries \cite{ferraiolo2017database}. The strongest software decision result is the linear-time graph algorithm of Mell et al.\ \cite{mell2017linear}. That is the right software bound, and it is still a per-decision traversal whose constant depends on the host. XEngine is the closest software analogue of our compilation step. It compiles XACML into numeric structures offline, but its decision latency still varies with policy and request shape \cite{liu2011xengine}. Reported measurements place OPA and an XACML PDP three to five orders of magnitude above the bound derived in Section~\ref{sec:arch} \cite{opa, karim2026tsngac, oasisxacml, servos2017abac}.

\textbf{Hardware enforcement.} Three lines of work are direct ancestors. Huffmire et al.\ compiled a formally specified memory-isolation policy into an FPGA reference monitor in 2006 \cite{huffmire2006policy}, and Fiorin et al.\ put access-rights checks into NoC interfaces with no added latency on the authorized path \cite{fiorin2008secure}. Both make enforcement latency a property of a synthesized circuit. Neither evaluates a relational policy model, and neither measures what an added policy dimension costs. The third is packet classification. TCAM and bit-vector engines match wide multi-field keys at deterministic rates \cite{song2005bvtcam}, and StrideBV advertises latency independent of ruleset content \cite{ganegedara2014scalable}. However, StrideBV latency grows linearly with match-key width. In the closest deterministic prior art, added dimensions are not free in time, which is the axis this paper claims. Aker wraps SoC controllers in verified access-control hardware at the bus-transaction level \cite{restuccia2021aker}, and CHERI enforces per-instruction memory capabilities in silicon \cite{watson2015cheri}. Both police a lower layer than a policy model.

\textbf{Real-time and robot security.} This literature schedules software enforcement around deadlines. Contego slots security tasks into scheduling slack \cite{hasan2017contego}, and a 2024 SoK catalogs the paradigm \cite{hasan2024sok}. We leave that paradigm rather than optimize within it, because a fabric decision consumes no CPU slack. Pinisetty et al.\ synthesize runtime enforcers from timed automata for reactive CPS \cite{pinisetty2017runtime}. That is the nearest formal-synthesis neighbor, and it targets safety properties rather than authorization with no hardware latency model. In robotics, Dieber et al.\ argued for application-layer authorization above transport crypto \cite{dieber2017security}, ROS-Defender filters node flows at the network layer \cite{rivera2019rosdefender}, and Salimi et al.\ condition ABAC decisions on task and capability attributes through a blockchain bridge \cite{salimi2025abac}. None conditions a command on its source type or the platform's runtime state, and none is an inline bounded-latency primitive.
...
\textbf{Unauthorized access.} The attacker acts as an unprivileged agent or through a hijacked session (CVE-2022-45789, Schneider Electric Modicon \cite{cve202245789}). The identity triple closes this class, which every access control model already addresses.
...
\textbf{Command provenance abuse.} The attacker holds valid DDS credentials for an authorized subject but is not an entitled source type for the command it issues (CVE-2021-38425, eProsima Fast DDS \cite{cve202138425}). SROS2 and DDS-Security authenticate who a node is at the transport layer \cite{mayoralvilches2022sros2, omgddssec}, but never ask whether that source type may issue that command to that resource. That question is invisible below the application layer by design. Further, transport access control is itself fragile: Deng et al.\ demonstrated four ways a credentialed insider bypasses SROS2 permissions and keeps publishing \cite{deng2022insecurity}.

A fourth concern is deliberately not an attack class. The timing window, in which a correct deny arrives after the actuator has acted, cuts across all three classes. It is closed by bounding decision latency below message propagation time, not by any policy dimension. Section~\ref{sec:results} returns to it.

\subsection{Assumptions}

We assume the attacker holds valid transport credentials for at least one authorized subject and can publish at wire rate. Transport security is intact, and we do not defend broken cryptography. The decision point and the compiled policy are trusted. Compilation happens off the decision path, and policy updates are outside our present scope. The platform exposes its runtime state to the decision point as a bit vector.
...
Forgoing early exit costs the average case. A software scan that exits on first match beats (4) whenever the match lands early. We spend the average case to buy a worst case with zero variance, because a control loop budgets against the worst case.
...
The same kernel logic compiled for the i7 and measured under perf tells the opposite story. Per-rule marginal cost rises from 0.645 cycles in three dimensions to 1.024 in four and 1.220 in five. In software every rule pays for every dimension on every decision, and the cost compounds with policy size. At 500 rules the five-dimensional software path spends 685 derived cycles per decision against the hardware kernel's 262 measured cycles. Figure~\ref{fig:keyfinding} plots both families. The software curves fan out as dimensions are added\new{, and the three hardware curves are one line}.
...
Four questions follow. \new{1.~At what clock does the next dimension stop being free, and does that ceiling move with policy width or only with stage depth? 2.~What does a software decision path on the board's own Cortex-A9 cost, so the wall-clock comparison runs at one class of silicon?} 3.~Does the closed form of (4) survive a dynamic policy update path, or does update traffic reintroduce jitter? 4.~How does the primitive compose with time-scoped delegation, which TS-NGAC develops in software \cite{karim2026tsngac}? Further, we call on the community to report authorization performance as cycle-level worst cases rather than means, because a mean is the statistic a control loop cannot budget against.
