...
% hyperref LAST, after every other package. LOADED IN DRAFT MODE FOR SUBMISSION
% 2026-08-20: the IPCCC upload checker rejects PDF bookmarks (FAQ 115) and URL
% link annotations (FAQ 221), and hidelinks only suppresses the visible frame,
% not the annotation itself. Draft mode turns every hypertext feature off, so no
% /Outlines tree and no /Annots array reach the PDF. Nothing in the document
% breaks: \ref, \eqref and \cite come from LaTeX and amsmath, and \url still
% typesets through the url package loaded above.
% For arXiv or a camera-ready that wants live links, restore:
...
% MARKUP OFF FOR SUBMISSION 2026-08-20. Both commands stay defined so every
% \new{...} and \newcolor call site keeps compiling untouched. To bring the red
% back for a revision pass, restore the two commented lines below and comment
% out the two live ones.
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/fig-scenario-Verticle.drawio.pdf}
\caption{\new{Authenticated but unauthorized. Three commands clear DDS/SROS2 authentication and arrive as equals. The five-dimensional kernel separates them, one on the state precondition the maintenance lock clears and one on provenance. A three-dimensional kernel permits all three.}}
\label{fig:scenario}
\end{figure}
...
A quadruped inspection robot in maintenance mode should not move. Layla, a field robotics engineer, is replacing a battery pack on a 45~kg rover when an "increase velocity" command arrives from a remote operator console. The Data Distribution Service (DDS) transport authenticates the console's certificate and delivers it in well under a millisecond. Nothing asks whether a remote operator may command motion while a human is at the rover and the platform is locked for maintenance. If the rover acts, Layla could be hurt. \new{Figure~\ref{fig:scenario} shows the path.} CVE-2022-33323~\cite{cve202233323} demonstrates this failure mode against Mitsubishi MELFA industrial robots, and CVE-2021-38425~\cite{cve202138425} documents message injection against eProsima Fast DDS, the default ROS~2 transport.
...
\textbf{Unsafe-state operation.} A malicious or confused client issues a well-formed command that the platform's current state does not license. \new{The same drive command is routine while the rover is operating and hazardous once a technician locks it for maintenance} (CVE-2022-33323, Mitsubishi MELFA \cite{cve202233323}). \new{The drive rule names the operating state as a precondition, and the maintenance lock clears it, so the command permitted a moment earlier is denied.} Identity-based authorization admits this class because the request is legitimate on every credential it carries.
...
is a bitwise AND against a precompiled mask followed by a comparison. The first three terms are the H-NGAC identity triple. The $\sigma$ term conditions the permission on the platform's runtime state, and the $\pi$ term conditions it on the command's source type. Dimensions four and five add two AND terms to a conjunction that already exists. They add no traversal, no lookup and no branch. \new{One convention governs $\sigma$. A rule names the states in which it may fire, so the encoding is positive and monotone. Asserting a state bit can only make more rules match, so the model denies on a missing precondition, not on a hazard flag being set.} On scope, we compile only the association subset of INCITS 565, that is, the rules that grant. Prohibitions, obligations and administrative relations are not compiled, and Section~\ref{sec:limitations} bounds our claims accordingly.
...
\toprule
& & Cycles & \multicolumn{3}{c}{\new{Requests permitted}} \\
\cmidrule(lr){3-3} \cmidrule(lr){4-6}
Rules & Requests & 3D\,=\,4D\,=\,5D & \new{3D} & \new{4D} & \new{5D} \\
\midrule
4 & 11 & 14 & \new{11} & \new{8} & \new{4} \\
10 & 27 & 17 & \new{27} & \new{19} & \new{10} \\
50 & 134 & 37 & \new{134} & \new{92} & \new{50} \\
...
\new{Table~\ref{tab:cycles} puts both halves of the result side by side. One corpus of 2{,}307 requests went through all three co-simulations, so every number in it is a decision taken by RTL rather than by a software model of it. Read the cycle column and the three kernels are one number. Read the permit columns and they are three.}
...
\new{The two halves move independently. That is the finding. In hardware, the security question a kernel can ask is decoupled from what asking it costs in time.}
...
% ---- OPA/XACML paragraph, CUT 2026-08-20 to pay for Fig. 1 and the
% sigma-semantics text. Both its numbers (271.491 us / 13 deadline misses,
% and the ~50 ms XACML figure) already appear in the Introduction, so no
% fact is lost. Uncomment to restore; costs one page.
...
\new{First, the parity result covers three, four and five dimensions on one part at one clock. We have not shown that an arbitrary sixth dimension is free, only that these two were, and Section~\ref{sec:arch} names the mechanism that would end the pattern.} \new{Second,} the software baseline is a 4.96~GHz laptop CPU while the fabric runs at 100~MHz. A baseline on the board's own Cortex-A9 would make the wall-clock comparison fairer, and we did not run it. \new{The board data bounds the fabric and the interface, not a software decision path on the same silicon.} \new{Third,} software latency distributions were measured under WSL2, whose scheduler jitter contaminates maxima, which is why we cite worst observed values and never call them WCETs. \new{Fourth,} the primitive is capacity-bounded at 512 rules and 256 policy nodes, and policy update happens off the decision path. \new{Fifth,} the modeled external-state lookup is synthetic and excluded from every empirical claim, since the measured SQLite path shows the same thing. \new{Sixth,} the kernel compiles only the association subset of INCITS 565, so no claim extends beyond rules that grant. \new{$\sigma$ and $\pi$ are likewise positive conditions: a rule requires a state or source type and cannot forbid one.} \new{Seventh,} the zero-cycle result holds at one operating point, 100~MHz on a Zynq-7020, where \new{all three} kernels close timing with +0.33~ns slack. At an aggressive clock target \new{an} added dimension could force another pipeline stage, the boundary the hypothesis of Section~\ref{sec:model} predicts\new{. The two-to-three step in iteration latency already shows that mechanism operating below the threshold where it costs cycles}. Characterizing the frequency ceiling is future work.

\section{Conclusion and Future Work}
\label{sec:conclusion}

% ORDER CORRECTED 2026-08-20. This section ran future work first from the v1
% scaffold onward. The finding now comes first and the open questions follow,
% so the last paragraph a reviewer reads is the answer and not the backlog.
% CONCLUSION REWRITTEN 2026-08-20. Structure: finding, evidence, significance.
% The v2 paragraph restated every headline number already in the abstract and in
% Sections VII-A through VII-E, so it read as a summary rather than an answer.
% Cut from it: the LUT count, the ROS 2 counts, and the synthesis narrative.
% All three survive in the abstract, which is where a skimming reviewer meets
% them first. Optional closing sentence, restores the Layla bookend from the
% Introduction at a cost of 21 words:
%   Layla's interlock becomes a precondition the fabric checks in bounded time, not a procedure she hopes the operator follows.
We've determined that in hardware, security dimensionality is free in time. On a Zynq-7020 at 100~MHz, \new{three-, }four- and five-dimensional H-NGAC kernels resolve every decision in the same number of clock cycles at every policy size, yet across one 2{,}307-request corpus the three-dimensional kernel permits \new{1{,}443} requests the five-dimensional kernel denies. Latency is closed-form at $12 + n/2$ cycles, zero jitter by construction\new{ and confirmed on silicon}. Area, not time, is what a dimension costs. Thus the security question a control path can afford to ask is no longer set by its latency budget. A robotics or functional-safety engineer fixes the authorization worst case at synthesis and spends the remaining margin on policy.

For future works, three questions remain outstanding: \new{1.~At what clock does the next dimension stop being free, and does that ceiling move with policy width or only with stage depth? 2.~What does a software decision path on the board's own Cortex-A9 cost, so the wall-clock comparison runs at one class of silicon?} 3.~Does \eqref{eq:latency} survive a dynamic policy update path, or does update traffic reintroduce jitter? %4.~How does the primitive compose with time-scoped delegation, which TS-NGAC develops in software \cite{karim2026tsngac}?
Further, we call on the community to report authorization performance as cycle-level worst cases rather than means, because a mean creates probabilistic uncertainty preventing tight control loop budgeting.
