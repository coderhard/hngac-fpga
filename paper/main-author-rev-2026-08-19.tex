...
% Pastel row tints for the related-work rubric (Table II). Chosen light enough
% that black text keeps >14:1 contrast on every one, and paired with printed group
% labels so no information is carried by color alone.
\definecolor{grpSW}{HTML}{E7F0F9}
\definecolor{grpHW}{HTML}{E8F1E3}
\definecolor{grpRT}{HTML}{FBEDDF}
\definecolor{grpOUR}{HTML}{F0EAF7}
\definecolor{grpTHIS}{HTML}{F9E39F}
...
\begin{abstract}
Real-time robotic and drone system designers often leave authorization out of the control path because software policy engines do not offer decisions within a fixed cycle budget. Operating system scheduling, not policy evaluation, creates non-deterministic latency, so a faster software-based engine does not offer the guarantees required by hard real-time applications. The gap leaves attack classes open that identity-based authorization cannot see, including unsafe-state operation and command injection from compromised but credentialed nodes. State of the art H-NGAC compiles NGAC policy graphs into fixed-width hardware bitmasks so that an authorization decision reduces to a chain of bitwise AND operations. In this paper, we improve hardware security controls by extending H-NGAC with two further dimensions, runtime system state and command provenance. We demonstrate using three-, four- and five-dimensional kernels on a Zynq-7020 FPGA. % We evaluated the synthesized kernels against six software baselines.
We also prove the security efficacy against the five-dimensional policy in an adversarial Robot Operating System 2 (ROS~2) scenario against a compromised node that has already authenticated. We found that the added security dimensions are free in time. All three kernels resolve in an identical number of clock cycles at every policy size tested, with a closed-form worst case and zero jitter by construction.
On the same corpus the three-dimensional kernel permits every request the five-dimensional kernel denies.
In software, the same added dimension makes every policy rule 19\% more expensive. Thus, the authorization worst case becomes a synthesis parameter rather than a measurement, and we quantify what that determinism costs in area and in average-case speed.
\end{abstract}
...
A quadruped inspection robot in maintenance mode should not move. Consider the scenario where Layla, a field robotics engineer, is replacing a battery pack on a 45~kg rover, and an "increase velocity" command arrives from a remote operator console. The Data Distribution Service middleware API protocol (DDS) transport authenticates the console's certificate and delivers the command in well under a millisecond. In this scenario, nothing asks whether a remote operator was entitled to command motion while a human is nearby and explicitly locked to maintenance mode. If the rover acts, Layla could be hurt. This failure mode is demonstrated in CVE-2022-33323~\cite{cve202233323} where Mitsubishi MELFA industrial robots were vulnerable to unauthorized command execution. Also CVE-2021-38425~\cite{cve202138425} documents message injection against eProsima Fast DDS, the default ROS~2 transport.



Robotics platforms leave authorization out of the control path for a defensible reason. Although modern IT enterprise policy engines evaluate rich attribute-based policy, they cannot guarantee decision timing. That is, the latency cost of a software policy decision point is governed by operating system scheduling, preemption and cache state, not by policy evaluation, and thus offer no bounds by which to plan. So, a faster engine may not offer faster results. In previous measurements \cite{karim2026dcas}, an OPA edge deployment took 1 to 5~ms per decision and an XACML decision point near 50~ms, while a software authorization path averaging 1.14~$\mu$s still produced a single 157~$\mu$s scheduling outlier. In a companion evaluation \cite{karim2026tsngac}, we measured OPA at a 271.491~$\mu$s mean with 13 missed real-time deadlines. A control loop cannot budget for a tail it cannot bound. It is for this reason that robotic system designers place authorization at the perimeter and leave the control path open.

Our research approaches this as a compilation problem. Hypergraph privilege analysis posited in \cite{sitharaman2025hypergraph} established that large privilege structures compile into forms that answer set-level questions quickly. %That insight is the intellectual origin of this work.
H-NGAC~\cite{karim2026dcas},  presented at IEEE DCAS 2026, converted that idea to the decision path timing. By compiling the NGAC policy graph \cite{incits565, ferraiolo2011policymachine} into fixed-width bitmasks they were able to force one authorization decision to be reduced to a chain of bitwise AND operations. H-NGAC, however, only checked the identity triple of subject, object and attribute. It did not model the platform's runtime state, and it did not model where a command came from. Layla's maintenance lock flag and a remote operators increase-velocity problem exists within the missing dimensions.
...
The remainder of this paper is organized as follows. Section~\ref{sec:related} rates related work on a scored rubric. Section~\ref{sec:background} describes the threat model and the background this work extends. Section~\ref{sec:model} formalizes the 5D H-NGAC decision function. Section~\ref{sec:arch} renders the model into a pipelined hardware architecture and derives its determinism. Section~\ref{sec:method} details the experimental method and its honesty rules. Section~\ref{sec:results} reports results. Section~\ref{sec:limitations} bounds our claims, and Section~\ref{sec:conclusion} concludes.
...
\begin{table}[t]
\caption{Related work scored on the four rubric axes. Group names are printed, so the shading is redundant encoding.}
\label{tab:rubric}
\centering
\footnotesize
...
\rowcolor{grpSW} NGAC standard / Policy Machine \cite{incits565, ferraiolo2011policymachine} & 1 & 0 & 2 & 0 \\
\rowcolor{grpSW} Linear-time NGAC \cite{mell2017linear} & 3 & 1 & 2 & 0 \\
\rowcolor{grpSW} XEngine \cite{liu2011xengine} & 3 & 0 & 3 & 0 \\
\rowcolor{grpSW} OPA \cite{opa} & 3 & 0 & 3 & 0 \\
\rowcolor{grpSW} XACML PDP \cite{oasisxacml} & 2 & 0 & 3 & 0 \\
\addlinespace[2pt]
\multicolumn{5}{l}{\textit{Hardware enforcement}} \\
\rowcolor{grpHW} FPGA reference monitor \cite{huffmire2006policy} & 3 & 3 & 1 & 4 \\
\rowcolor{grpHW} NoC data protection units \cite{fiorin2008secure} & 3 & 3 & 1 & 3 \\
\rowcolor{grpHW} Bit-vector classification \cite{song2005bvtcam, ganegedara2014scalable} & 4 & 4 & 0 & 4 \\
\rowcolor{grpHW} SoC access-control wrappers \cite{restuccia2021aker} & 3 & 3 & 1 & 4 \\
\rowcolor{grpHW} CHERI \cite{watson2015cheri} & 4 & 4 & 0 & 5 \\
\addlinespace[2pt]
\multicolumn{5}{l}{\textit{Real-time and robot security}} \\
\rowcolor{grpRT} SROS2 / DDS-Security \cite{mayoralvilches2022sros2, omgddssec} & 2 & 0 & 0 & 0 \\
\rowcolor{grpRT} App-layer ROS authorization \cite{dieber2017security} & 2 & 0 & 1 & 0 \\
\rowcolor{grpRT} ROS-Defender \cite{rivera2019rosdefender} & 2 & 0 & 1 & 0 \\
\rowcolor{grpRT} ABAC for multi-robot systems \cite{salimi2025abac} & 2 & 0 & 3 & 0 \\
\rowcolor{grpRT} Security-aware RT scheduling \cite{hasan2017contego, lesi2017security, hasan2024sok} & 3 & 2 & 1 & 0 \\
\rowcolor{grpRT} CPS runtime enforcement \cite{pinisetty2017runtime} & 2 & 3 & 1 & 0 \\
\addlinespace[2pt]
\multicolumn{5}{l}{\textit{Our prior work}} \\
\rowcolor{grpOUR} HyperNGAC \cite{sitharaman2025hypergraph} & 0 & 0 & 3 & 0 \\
\rowcolor{grpOUR} H-NGAC \cite{karim2026dcas} & 4 & 2 & 0 & 1 \\
\rowcolor{grpOUR} TS-NGAC \cite{karim2026tsngac} & 4 & 3 & 3 & 0 \\
\addlinespace[2pt]
\rowcolor{grpTHIS} \textbf{This work} & \textbf{5} & \textbf{5} & \textbf{5} & \textbf{5} \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Software policy engines and models.} The NGAC standard and the Policy Machine define the model we compile but report no per-decision latency evidence \cite{incits565, ferraiolo2011policymachine, hu2014sp800162}, and NGAC's decision and review advantages over XACML are established in the standards comparison of Ferraiolo et al.\ \cite{ferraiolo2016xacml}. NGAC implementations to date are software, including NIST's own fine-grain enforcement over database queries \cite{ferraiolo2017database}, and the strongest software NGAC decision result is the linear-time graph algorithms of Mell et al.\ \cite{mell2017linear}. That is the right software bound, and it is still a per-decision traversal whose constant depends on the host. XEngine compiles XACML into numeric structures offline and speeds decisions by orders of magnitude \cite{liu2011xengine}, the closest software analogue of our compilation step, but its decision latency still varies with policy and request shape. Servos and Osborn's survey lists ABAC decision performance among the field's open problems \cite{servos2017abac}, and our own measurements place OPA and an XACML PDP three to five orders of magnitude above the closed-form bound we derive in Section~\ref{sec:arch} \cite{opa, karim2026tsngac, oasisxacml}.
...
Since this work measures cycle-accurate per-decision latency, verifies a closed-form bound at every measured point, enforces both state and provenance, and validates through co-simulation, silicon functional verification, \new{and a hardware-latched on-board cycle capture that tracks co-simulation to a constant offset}, we assign it the maximum on all four axes under the definitions above, and Section~\ref{sec:results} supplies the evidence for that row. We state the novelty claim in its bounded form. Constant-latency wide-key matching exists in TCAMs, deterministic classification exists in the bit-vector family, and policy-to-circuit compilation dates to 2006. To our knowledge, no prior work compiles a standardized relational authorization model to hardware and demonstrates, on identical toolflow and part, that added policy dimensions cost zero clock cycles, and no prior hardware implementation of NGAC exists at all. We invite correction on both points.
...
A fourth concern is deliberately not an attack class. The timing window, in which a correct deny arrives after the actuator has already acted, is a delivery property that cuts across all three classes. It is closed by bounding decision latency below message propagation time, not by any policy dimension. We return to it in Section~\ref{sec:results}.
