...
\title{Authenticated but Unauthorized: 5D H-NGAC for Low-Latency Authorization in Real-Time Robotics}
...
\begin{itemize}
    ...
    \item A synthesized hardware evaluation on a Zynq-7020 showing three-, four- and five-dimensional kernels with equal clock cycles. % SAVE THIS FOR RESULTS... at every policy size tested, with a closed-form latency of $12 + n/2$ cycles for $n$ rules, zero jitter by construction, \new{and on-board confirmation from a hardware-latched counter, }while the \new{two added dimensions together cost 1{,}336 LUTs} and no BRAM or DSP; and
...
%The central finding deserves its own sentence.
% MOVE THIS TO RESULTS... In software, each added security dimension makes every policy rule more expensive, raising the per-rule cost from 0.645 to 1.220 cycles between the three- and five-dimensional variants. In hardware the added dimensions cost zero cycles. They are free in time and nearly free in area, which converts the authorization worst case from a measurement into a synthesis parameter.

The remainder of this paper is organized as follows. Section~\ref{sec:background} describes the threat model and the background this work extends. Section~\ref{sec:model} formalizes the 5D H-NGAC decision function. Section~\ref{sec:arch} renders the model into a pipelined hardware architecture and derives its determinism. Section~\ref{sec:method} details the experimental method and its honesty rules. Section~\ref{sec:results} reports results. Section~\ref{sec:related} rates related work on a scored rubric. Section~\ref{sec:limitations} bounds our claims, and Section~\ref{sec:conclusion} concludes.
